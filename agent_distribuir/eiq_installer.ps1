<#
.SYNOPSIS
    EndpointIQ Agent - Instalador Universal para Windows 10/11
.DESCRIPTION
    Instala el agente de monitoreo EndpointIQ con auto-actualizacion.
...
param(
    [string]$InstallDir = "C:\ProgramData\EndpointIQ",
...
$TASK_NAME = "EndpointIQ-Agent"
$AGENT_SCRIPT = "eiq_agent.py"
$SERVER_URL = "https://endpointiq-175647544738.us-central1.run.app"

# --- Banner ---
Write-Host ""
Write-Host "  +==================================================+"
Write-Host "  |     EndpointIQ Agent - Instalador v2.0           |"
...
foreach ($candidate in $searchPaths) {
    try {
        ...
        if ($resolvedPath) {
            $verFile = Join-Path $env:TEMP "eiq_pyver.txt"
            $verErr = Join-Path $env:TEMP "eiq_pyver_err.txt"
...
# Limpiar temporales
Remove-Item (Join-Path $env:TEMP "eiq_pyver.txt") -Force -ErrorAction SilentlyContinue
Remove-Item (Join-Path $env:TEMP "eiq_pyver_err.txt") -Force -ErrorAction SilentlyContinue
...
$fileList = @("eiq_agent.py", "eiq_config.json", "eiq_credentials.json", "eiq_launcher.vbs")
$copiedCount = 0
...
$configPath = Join-Path $InstallDir "eiq_config.json"
...
foreach ($pkg in $pipPkgs) {
    ...
    $pipOut = Join-Path $env:TEMP "eiq_pip_out.txt"
    $pipErr = Join-Path $env:TEMP "eiq_pip_err.txt"
...
try {
    $existingRule = Get-NetFirewallRule -DisplayName "EndpointIQ Agent" -ErrorAction SilentlyContinue
    if ($existingRule) {
        Remove-NetFirewallRule -DisplayName "EndpointIQ Agent" -ErrorAction SilentlyContinue
    }
    New-NetFirewallRule -DisplayName "EndpointIQ Agent" -Direction Outbound -Action Allow -Program $pythonExe -Protocol TCP -RemotePort 443 -ErrorAction Stop | Out-Null
...
$vbsLauncher = Join-Path $InstallDir "eiq_launcher.vbs"
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>EndpointIQ - Monitoreo invisible cada $IntervalMinutes min</Description>
  </RegistrationInfo>
  <Triggers>
    <TimeTrigger>
      <Repetition>
...
$xmlPath = Join-Path $env:TEMP "eiq_task.xml"
...
$testOut = Join-Path $env:TEMP "eiq_test.txt"
$testErr = Join-Path $env:TEMP "eiq_test_err.txt"
try {
...
Write-Host "  |  endpointiq-175647544738.us-central1.run.app     |" -ForegroundColor Cyan
Write-Host "  +==================================================+" -ForegroundColor Green
Write-Host ""
