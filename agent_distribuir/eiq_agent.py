...
def collect_metrics():
    ...
    network_info = {"interfaces": [], "wifi_ssid": None, "connected_devices": []}
    try:
        # WiFi SSID - Method 1: netsh
        try:
            ...
            for line in wifi_out.splitlines():
                ls = line.strip()
                if "SSID" in ls and "BSSID" not in ls:
                    parts = ls.split(":", 1)
                    if len(parts) == 2 and parts[1].strip():
                        network_info["wifi_ssid"] = parts[1].strip()
                        break
        except Exception:
            pass

        # WiFi SSID - Method 2: PowerShell fallback
        if not network_info["wifi_ssid"]:
            try:
                ps_out = subprocess.check_output(
                    ["powershell", "-Command", "(Get-NetConnectionProfile).Name"],
                    timeout=5, stderr=subprocess.DEVNULL
                ).decode("utf-8", errors="replace").strip()
                if ps_out and ps_out not in ("", "Identificando"):
                    network_info["wifi_ssid"] = ps_out
            except Exception:
                pass
        ...
        for iface, addrs in net_addrs.items():
            ...
            if not ipv4 or ipv4.startswith("127."):
                continue
            io = net_io.get(iface)
            conn_type = "WiFi" if any(k in iface.lower() for k in ["wi-fi", "wlan", "wireless"]) else "Ethernet"
            network_info["interfaces"].append({
                "name": iface, "ip": ipv4, "mac": mac, "type": conn_type,
                "speed_mbps": stat.speed if stat.speed > 0 else None,
                "bytes_sent": io.bytes_sent if io else 0,
                "bytes_recv": io.bytes_recv if io else 0
            })

        # ARP table for connected devices (filter multicast/broadcast)
        try:
            arp_out = subprocess.check_output(
                ["arp", "-a"], timeout=5, stderr=subprocess.DEVNULL
            ).decode("utf-8", errors="replace")
            for line in arp_out.splitlines():
                ...
                if len(parts) >= 3 and parts[0].count('.') == 3:
                    ...
                    if ip_addr.startswith("224.") or ip_addr.startswith("239.") or ip_addr.startswith("255.") or ip_addr.endswith(".255"):
                        continue
                    network_info["connected_devices"].append({"ip": ip_addr, "mac": mac_addr, "type": dev_type})
            network_info["connected_devices"] = network_info["connected_devices"][:20]
        except Exception:
            pass

    except Exception as e:
        log.warning("[COLLECT] Network info error: %s", e)

    metrics_row["network_info"] = json.dumps(network_info, default=str)
    log.info("[COLLECT] Network: SSID=%s Ifaces=%d Devices=%d",
             network_info.get("wifi_ssid", "N/A"),
             len(network_info.get("interfaces", [])),
             len(network_info.get("connected_devices", [])))
