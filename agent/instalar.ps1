...
# -----------------------------------------------
Write-Host "  [5/8] Verificando configuración del agente..." -ForegroundColor Yellow
$configFile = "$InstallDir\onyx_config.json"
if (-not (Test-Path $configFile) -and (Test-Path "$SourceDir\onyx_config.json")) {
    Copy-Item "$SourceDir\onyx_config.json" $configFile -Force
}
if (-not (Test-Path $configFile)) {
    $jsonContent = [PSCustomObject]@{
        ...
        agent_secret        = "onyx-shared-secret-v350-2026"
    }
    $jsonStr = $jsonContent | ConvertTo-Json -Depth 3
    [System.IO.File]::WriteAllText($configFile, $jsonStr, [System.Text.Encoding]::UTF8)
    Write-Host "        [OK] Config generada por defecto" -ForegroundColor Green
} else {
    Write-Host "        [OK] Config aplicada exitosamente desde el paquete del servidor" -ForegroundColor Green
}
Write-Host ""
...
# -----------------------------------------------
# PASO 7: Exclusion Defender + Tarea programada + Inicio automatico
# -----------------------------------------------
...
# Limpieza de startups anteriores no validos
$startupFolder = [System.Environment]::GetFolderPath([System.Environment+SpecialFolder]::Startup)
$startupVbs = Join-Path $startupFolder "OnyxAgent.vbs"
if (Test-Path $startupVbs) { Remove-Item $startupVbs -Force -ErrorAction SilentlyContinue }

# Crear acceso directo de inicio limpio
$vbsContent = "Set WshShell = CreateObject(`"WScript.Shell`")`r`nWshShell.Run `"wscript.exe `"`"$launcher`"`"`, 0, False`r`n"
[System.IO.File]::WriteAllText($startupVbs, $vbsContent, [System.Text.Encoding]::ASCII)
Write-Host "        [OK] Inicio automatico de Windows configurado en Startup" -ForegroundColor Green
...
# -----------------------------------------------
# PASO 8: Primera ejecucion y arranque en segundo plano
# -----------------------------------------------
Write-Host "  [8/8] Ejecutando primera recoleccion y arrancando agente..." -ForegroundColor Yellow
Write-Host "        [..] Enviando metricas iniciales al servidor..."
$proc = Start-Process -FilePath $pythonExe -ArgumentList "`"$InstallDir\onyx_agent.py`" --once" -Wait -PassThru -NoNewWindow 2>$null
if ($proc.ExitCode -eq 0) {
    Write-Host "        [OK] Telemetría enviada correctamente" -ForegroundColor Green
} else {
    Write-Host "        [WARN] Primera recolección en proceso. El agente reintentará automáticamente." -ForegroundColor Yellow
}

# Arrancar la tarea en segundo plano inmediatamente
schtasks /run /tn "Onyx-Agent" 2>&1 | Out-Null
Start-Process -FilePath "wscript.exe" -ArgumentList "`"$launcher`"" -NoNewWindow 2>$null
Write-Host "        [OK] Agente iniciado en segundo plano exitosamente" -ForegroundColor Green
Write-Host ""
