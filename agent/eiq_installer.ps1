<#
.SYNOPSIS
    EndpointIQ Agent - Instalador para Windows 10/11
.DESCRIPTION
    Instala el agente de monitoreo EndpointIQ.
...
Write-Host ""
Write-Host "  +==================================================+"
Write-Host "  |       EndpointIQ Agent - Instalador v1.0         |"
...
$fileList = @("eiq_agent.py", "eiq_config.json", "eiq_credentials.json")
...
$pipLog1 = Join-Path $env:TEMP "eiq_pip1.txt"
Start-Process -FilePath $pythonExe -ArgumentList "-m pip install --quiet --upgrade psutil" -NoNewWindow -Wait -RedirectStandardOutput $pipLog1 -RedirectStandardError "$env:TEMP\eiq_pip1_err.txt"

Write-Host "    [+] google-cloud-bigquery..." -ForegroundColor Gray
$pipLog2 = Join-Path $env:TEMP "eiq_pip2.txt"
Start-Process -FilePath $pythonExe -ArgumentList "-m pip install --quiet --upgrade google-cloud-bigquery" -NoNewWindow -Wait -RedirectStandardOutput $pipLog2 -RedirectStandardError "$env:TEMP\eiq_pip2_err.txt"

# Limpiar logs temporales de pip
Remove-Item "$env:TEMP\eiq_pip*.txt" -Force -ErrorAction SilentlyContinue
...
$taskDescription = "EndpointIQ - Agente de monitoreo cada $IntervalMinutes minutos."
...
Write-Host "  |  Plataforma: endpointiq-175647544738             |" -ForegroundColor Cyan
